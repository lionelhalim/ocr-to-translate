...
class App(QWidget):
    def __init__(self):
        ...
        self.init_ui()
        # Ctrl+Shift+S to select and capture
        keyboard.add_hotkey('ctrl+shift+alt+d', lambda: self.signals.capture.emit())
        # Ctrl+Shift+L to re-capture last region
        keyboard.add_hotkey('ctrl+shift+l', lambda: self.signals.screenshot_last.emit())

    def init_ui(self):
        self.setWindowTitle('Screenshot OCR & Translation')
        layout = QVBoxLayout()
        layout.addWidget(QLabel('(ctrl+shift+alt+d) Last Screenshot:'))
...
if __name__=='__main__':
    ...
    app=QApplication(sys.argv); ex=App(); ex.show(); sys.exit(app.exec_())